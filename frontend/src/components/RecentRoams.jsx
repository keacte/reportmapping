    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          className="pointer-events-auto absolute left-1/2 top-20 z-30 w-[26rem] -translate-x-1/2 glass rounded-sm p-4"
          data-testid="recent-roams-panel"
        >
          <div className="flex items-center justify-between">
            <div className="font-mono text-[10px] uppercase tracking-[0.25em] text-cyan-400">Recent Roams</div>
            ...
            {(reports || []).map((r) => {
              ...
              return (
                <button
                  ...
                  onClick={() => onPick(r.id)}
                  className={`group flex w-full items-center gap-3 border p-2 text-left transition-all ${active ? "border-cyan-400/60 bg-cyan-500/10" : "border-white/10 bg-white/[0.02] hover:border-cyan-400/40 hover:bg-white/[0.04]"}`}
                >
                  {r.hostLogo ? (
                  ...
                  <div className="shrink-0 text-right">
                    <div className="font-mono text-xs font-semibold text-amber-300">{r.iskHuman}</div>
                    <ChevronRight className="ml-auto h-3.5 w-3.5 text-cyan-400 opacity-0 transition-opacity group-hover:opacity-100" />
                  </div>
                </button>
              );
            })}
...
function MiniStat({ label, value }) {
  return (
    <div className="text-center">
      <div className="font-display text-base font-bold tabular-nums text-cyan-300">{value ?? "–"}</div>
